...
class Dataset(object):
    def __init__(self, train_path, test_path, valid_path=None, graph_path=None, test_candidates_path=None,
        ...
        train = self._load_data_file(train_path)
        test = self._load_data_file(test_path)
        if valid_path is None:
            valid = self._load_valid_data_file(test_path, ratio=0.05)
        else:
            valid = self._load_data_file(valid_path)
        # test = self._load_data_file(test_path)
        ...
        #     test_by_rel = {os.path.basename(path).split('_')[-1]: self._load_data_file(path)
...
#             self.reversed_rel_dct = self._get_reversed_relation_dict(self.relation2id, get_reverse=get_reverse)
    ...
    def _load_data_file(self, filepath, max_dataloader=2560):
        data_tri = []
        count = 0
        with open(filepath) as fin:
            for l in fin:
                ...
                u_id, item_ids = inters[0], inters[1:]
                for i_id in item_ids:
                    data_tri.append((u_id, i_id + 22363, 0))  # 0表示购买行为
                if count == max_dataloader:
                    break
        return data_tri

    def _load_valid_data_file(self, filepath, ratio=0.05, max_dataloader=2560):
        num = int(max_dataloader * ratio)
        data_tri = []
        count = 0
        with open(filepath) as fin:
            for l in fin:
    ...
    def _convert_to_t_id(self, triples):
        return np.array([(self.entity2id[h], self.entity2id[t]) for h, t in triples], dtype='int32')
...
class Beauty(Dataset):
    path = '../data/KBC/Beauty'

    def __init__(self):
        self.name = 'Beauty'
        train_path = os.path.join(self.path, 'train_labels.txt')
        # valid_path = os.path.join(self.path, 'dev.txt')
        valid_path = None
        test_path = os.path.join(self.path, 'test_labels.txt')
        graph_path = os.path.join(self.path, 'kg_triples.txt')
        super(Beauty, self).__init__(train_path, test_path, valid_path, graph_path)
