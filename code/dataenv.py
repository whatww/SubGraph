...
class Graph(object):
    ...
    def make_temp_edges(self, batch, remove_all_head_tail_edges=True):
        ...
        if remove_all_head_tail_edges:
            ...
            if self.reversed_rel_dct is None:
                ...
                edges_idx = [i for i, (eid, h, t, r) in enumerate(self.full_edges)
                             if (h, t) not in batch_set and (t, h, self.reversed_rel_dct.get(r, -1)) not in batch_set]
    ...
    def get_candidate_edges(self, attended_nodes=None, tc=None):
        ...
        if attended_nodes is None:
            ...
            candidate_edges = np.concatenate([np.expand_dims(new_eg_idx, 1),
                                              self.full_edges[candidate_idx]],
                                             axis=1)  # (eg_idx, edge_id, vi, vj, rel) sorted by (eg_idx, edge_id)
    ...
    def get_union_edges(self, scanned_edges, selfloop_edges, tc=None):
        ...
        scanned_edges = np.zeros((0, 4), dtype='int32') if len(scanned_edges) == 0 else scanned_edges[:,
                                                                                        :4]  # (eg_idx, vi, vj, rel)
    ...
    def add_nodes_to_memorized(self, selected_edges, inplace=False, tc=None):
        ...
        if len(selected_edges) > 0 and len(new_nodes) > 0:
            memorized_and_new = np.concatenate([self.memorized_nodes, new_nodes],
                                               axis=0)  # n_memorized_and_new_nodes x 2
    ...
    def set_index_over_nodes(self, selected_edges, nodes, tc=None):
        ...
        new_idx_e2vi = np.expand_dims(np.arange(mask_vi.shape[0])[mask_vi], 1).astype(
            'int32')  # n_matched_by_idx_and_vi x 1
        new_idx_e2vj = np.expand_dims(np.arange(mask_vj.shape[0])[mask_vj], 1).astype(
            'int32')  # n_matched_by_idx_and_vj x 1
    ...
    def get_seen_edges(self, seen_nodes, aug_scanned_edges, tc=None):
        ...
        aug_scanned_vj = aug_scanned_edges[:,
                         [0, 2]].copy()  # n_aug_scanned_edges x 2, (eg_idx, vj) not unique and not sorted
        mask_vj = np.in1d(aug_scanned_vj.view('<i4,<i4'), seen_nodes.view('<i4,<i4'))
        seen_edges = aug_scanned_edges[mask_vj][:,
    ...
    def get_vivj_edges(self, vi_nodes, vj_nodes, with_eg_idx=True):
        ...
        if with_eg_idx:
            candidate_idx, new_eg_idx = groupby_1cols_merge(vi_nodes[:, 0], vi_nodes[:, 1],
                                                            self.edges[:, 1], self.edges[:, 0])
            candidate_edges = np.concatenate([np.expand_dims(new_eg_idx, 1), self.full_edges[candidate_idx]],
                                             axis=1)  # (eg_idx, edge_id, vi, vj, rel) sorted by (eg_idx, edge_id)
            candidate_vj = candidate_edges[:,
                           [0, 3]].copy()  # n_candidate_edges x 2, (eg_idx, vj) not unique and not sorted
            mask_vj = np.in1d(candidate_vj.view('<i4,<i4'), vj_nodes.view('<i4,<i4'))
            vivj_edges = candidate_edges[
                mask_vj]  # n_vivj_edges x 5, (eg_idx, edge_id, vi, vj, rel) sorted by (eg_idx, vi, vj)
        else:
...
class DataFeeder(object):
    def get_train_batch(self, train_data, graph, batch_size, shuffle=True, remove_all_head_tail_edges=True):
        ...
        while start < n_train:
            ...
            batch = np.array([train_data[i] for i in np.concatenate([rand_idx[start:end], rand_idx[:pad]])],
                             dtype='int32')
...
class DataEnv(object):
    ...
    def get_train_data(self, remove_all_head_tail_edges=True):
        return partial(self.data_feeder.get_train_batch, self.train[1280:], self.graph,
                       remove_all_head_tail_edges=remove_all_head_tail_edges)

    def get_setGraph(self):
        self.graph.use_full_edges()

    def get_trainGraph(self, batch):
        self.graph.make_temp_edges(batch, remove_all_head_tail_edges=True)
