...
def node2edge_op(inputs, selected_edges, return_vi=True, return_vj=True):
    ...
    idx = tf.cond(tf.equal(batch_size, 1), lambda: tf.zeros((n_selected_edges,), dtype='int32'),
                  lambda: selected_edges[:, 0])
    result = []
    if return_vi:
...
def aggregate_op(inputs, selected_edges, output_shape, at='vj', aggr_op_name='mean_v3'):
    ...
    if at == 'vi':
        ...
        if aggr_op_name == 'mean_v2':
            edge_count = tf.math.unsorted_segment_sum(tf.ones((len(idx_vj), 1)), idx_vj,
                                                      max_idx_vj + 1)  # (max_idx_vj+1) x 1
        ...
        idx_and_vj = tf.cast(tf.math.unsorted_segment_max(idx_and_vj, idx_vj, max_idx_vj + 1),
                             tf.int32)  # (max_idx_vj+1) x 2
...
def aggregate_v2_op(inputs, selected_edges, output_shape, at='vj', aggr_op_name='mean_v3'):
    ...
    if at == 'vi':
        ...
        if aggr_op_name == 'mean_v2':
            ...
            edge_vec_aggr = edge_vec_aggr * tf.math.sqrt(edge_count)  # (max_idx_vj+1) x n_dims
        new_idx_e2vj = selected_edges[:, 7]  # n_selected_edges
        reduced_idx_e2vj = tf.cast(tf.math.unsorted_segment_max(new_idx_e2vj, idx_vj, max_idx_vj + 1),
                                   tf.int32)  # (max_idx_vj+1)
...
class UnconsciousnessFlow(keras.Model):
    ...
    def call(self, inputs, selected_edges=None, shared_embedding=None, training=False, tc=None):
        ...
        message = self.message_fn((hidden_vi, rel_emb, hidden_vj), training=training)  # n_selected_edges x n_dims
        ...
        update = self.hidden_fn((message_aggr, hidden, ent_emb), training=training)  # 1 x n_nodes x n_dims
...
class ConsciousnessFlow(keras.Model):
    def __init__(self, n_entities, n_dims, n_layers, aggr_op_name):
        ...
        self.intervention_fn = keras.layers.Dense(self.n_dims, activation=None, use_bias=False,
                                                  name='intervention_fn')

    def call(self, inputs, seen_edges=None, memorized_nodes=None, node_attention=None, hidden_uncon=None,
             query_head_emb=None,
             query_rel_emb=None,
             shared_embedding=None, training=False, tc=None):
        ...
        message = self.message_fn((hidden_vi, rel_emb, hidden_vj, query_head_vec),
                                  training=training)  # n_seen_edges x n_dims
        ...
        update = self.hidden_fn((message_aggr, hidden, hidden_uncon, query_head_vec, query_rel_vec),
                                training=training)  # n_memorized_nodes x n_dims
    ...
    def call(self, inputs, scanned_edges=None, hidden_uncon=None, hidden_con=None, shared_embedding=None,
        ...
        if new_idx_for_memorized is not None:
            hidden_con = tf.scatter_nd(new_idx_for_memorized, hidden_con,
                                       tf.TensorShape((n_memorized_and_scanned_nodes,
                                                       self.n_dims)))  # n_memorized_and_scanned_nodes x n_dims_sm
...
class Model(tf.keras.Model):
    ...
    def initialize(self, training=False, output_attention=False, analyze_attention=False, tc=None):
        query_head_emb, query_rel_emb = self.shared_embedding.get_query_context(self.heads, self.rels)  # batch_size x n_dims
        #query_head_emb = self.shared_embedding.get_query_context(self.heads)  # batch_size x n_dims
        ...
        if self.hparams.uncon_steps is not None:
            for _ in range(self.hparams.uncon_steps):
                ...
                hidden_uncon = self.uncon_flow(hidden_uncon, selected_edges=selected_edges,
                                               ...
                                               tc=get(tc, 'model'), training=training)  # 1 x n_nodes x n_dims
        ...
        memorized_v = self.graph.set_init_memorized_nodes(
            self.heads)  # n_memorized_nodes (=batch_size) x 2, (eg_idx, v)
    ...
    def flow(self, hidden_uncon, hidden_con, node_attention, query_head_emb,
             query_rel_emb,
             training=False, output_attention=False, analyze_attention=False, tc=None):
        """ hidden_uncon: 1 x n_nodes x n_dims
            hidden_con: n_memorized_nodes x n_dims
            node_attention: batch_size x n_nodes
        """
        ...
        attended_nodes = self.graph.get_topk_nodes(node_attention, max_attended_nodes,
        ...
        aug_scanned_edges = self.graph.set_index_over_nodes(aug_scanned_edges, memorized_and_scanned,
                                                            tc=get(tc, 'graph'))

        new_node_attention = self.att_flow.call(node_attention,
        ...
        seen_nodes = self.graph.get_topk_nodes(new_node_attention, max_seen_nodes,
        ...
        if new_idx_for_memorized is not None:
            hidden_con = tf.scatter_nd(new_idx_for_memorized, hidden_con,
                                       tf.TensorShape((n_memorized_and_seen_nodes,
                                                       self.hparams.n_dims)))  # n_memorized_nodes (new) x n_dims
        ...
        new_hidden_con = self.con_flow(hidden_con,
                                       ...
                                       query_rel_emb=query_rel_emb,
                                       shared_embedding=self.shared_embedding,
                                       tc=get(tc, 'model'), training=training)  # n_memorized_nodes (new) x n_dims
    ...
    def save_attention_to_file(self, final_node_attention, id2entity, id2relation, epoch, dir_name, training=True):
        ...
        for batch_i in range(batch_size):
            ...
            head, tail = self.heads[batch_i], self.tails[batch_i]
            filename = '{:d}-_{:d}.txt'.format(head, tail)
            ...
            with open(os.path.join(dir_name, filename), 'w') as fout:
                ...
                for i, nds_vi in enumerate(nodes[:-1]):
                    nds_vj = nodes[i + 1]
                    edges = self.graph.get_vivj_edges(nds_vi, nds_vj,
                                                      with_eg_idx=False)  # n_vivj_edges x 4, (edge_id, vi, vj, rel) sorted by (vi, vj)
